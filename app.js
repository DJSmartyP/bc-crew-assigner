...
function adminGlobalDeploymentRow(m){
  ...
  return `<article class="admin-global-deployment-row admin-selected-deployment">${missionCard(m,true)}<section class="admin-selected-detail"><div class="admin-selected-detail-head"><div><span>Selected deployment</span><h3>Crew and allocation</h3></div><div class="admin-inline-metrics"><div><small>Seats filled</small><strong>${m.responseCount}/${capacity}</strong></div><div><small>First choices</small><strong>${firstChoices}</strong></div><div><small>Last-resort roles</small><strong class="${lastResort?"warn":""}">${lastResort}</strong></div></div></div>${adminGlobalRoleRoster(m)}<details class="admin-global-players"><summary>Player preferences and assignment detail <span>${m.responseCount}</span></summary><div class="admin-assignment-list">${adminPlayerAssignmentRows(m)}</div></details></section></article>`;
}
