...
function adminUnassignedPlayers(m){
  ...
  const capacity=(m.ships?.length||1)*MAX_PER_SHIP;
  const rows=unassigned.map(p=>{
    const reason=plan?.error?"Waiting for a valid allocation":players.length>capacity?"Waiting list — deployment capacity reached":"Not currently assigned";
...
async function renderAdminDashboard(){
  ...
  function bindMissionActions(scope=document){
    ...
    scope.querySelectorAll("[data-delete-mission]").forEach(b=>{b.onclick=async()=>{if(confirm("Delete this deployment and all player responses?"))await deleteMissionCascade(b.dataset.deleteMission);};});
  }
  bindMissionActions();
