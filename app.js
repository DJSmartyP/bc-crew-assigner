...
const pref=(p.prefs||[]).map(x=>x===FLEX?"No preference":x).join(" → ");
const ship=p.shipPref?(activeMission.ships||[]).findIndex(s=>s.id===p.shipPref):-1;
const assignment=ov?.role?`Locked: ${ov.role}`:(p.assignment?.role||p.assignment?.station||"Awaiting assignment");
const shipText=ship>=0?displayShip(activeMission.ships[ship],ship):(p.assignment?.ship||"No ship");
return `<details class="response-row">
<summary class="response-top"><div class="response-name">${esc(p.name)}</div><div class="response-meta">${esc(shipText)} · ${esc(assignment)}</div></summary>
<div class="response-details"><div class="detail-grid"><div><div class="label">Preferences</div><p>${esc(pref||"No preferences")}</p></div><div><div class="label">Ship preference</div><p>${ship>=0?esc(displayShip(activeMission.ships[ship],ship)):"No preference"}</p></div></div>
${ov?.role?`<div class="fixed-note">Locked: ${esc(ov.role)}</div>`:""}
<div class="actions"><button class="btn ghost tiny" data-edit-player="${p.id}">Edit</button><button class="btn danger tiny" data-delete-player="${p.id}">Delete</button></div></div></details>`;
}
