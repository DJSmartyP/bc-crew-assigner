...
async function openMissionSetup(existing=null){
  ...
  $("#missionSetupForm").onsubmit=async e=>{
    ...
    const payload={title:$("#missionName").value.trim(),date:$("#missionDate").value,shipCount:n,ships,balanceShips:n===2?balanceShips:false,closed:existing?.closed||false,overrides:existing?.overrides||{},ownerUid,ownerName:owner.name,inviteSlug,updatedAt:serverTimestamp()};
    try{
...
function adminGlobalDeploymentRow(m){
  const search=`${missionTitle(m)} ${m.ownerName||""} ${m.ownerEmail||""} ${deploymentShipSummary(m)} ${m.inviteSlug||""}`.toLowerCase();
  const capacity=(m.ships?.length||1)*MAX_PER_SHIP,firstChoices=m.adminPlan?.metrics?.first||0,lastResort=m.adminPlan?.metrics?.avoid||0;
  return `<article class="admin-global-deployment-row" data-admin-search="${esc(search)}">${missionCard(m,true)}<details class="admin-roster-disclosure"><summary><div class="admin-roster-summary-copy"><span>Live allocation</span><b>View crew by ship and station</b><div class="admin-inline-metrics"><div><small>Seats filled</small><strong>${m.responseCount}/${capacity}</strong></div><div><small>First choices</small><strong>${firstChoices}</strong></div><div><small>Last-resort roles</small><strong class="${lastResort?"warn":""}">${lastResort}</strong></div></div></div><div class="admin-roster-summary-meta"><span class="admin-view-hint">Open roster</span></div></summary>${adminGlobalRoleRoster(m)}</details></article>`;
}
...
async function renderAdminDashboard(){
  ...
  main.innerHTML=`<div class="page-head admin-page-head"><div><div class="eyebrow">Administrator</div><h1>Control centre</h1><p class="sub">Manage deployments first, then move into organiser accounts only when you need them.</p></div><button id="adminCreateMissionBtn" class="btn primary">Create deployment</button></div><div class="admin-summary-grid"><div class="admin-summary-card"><span>Deployments</span><b>${missions.length}</b><small>${openDeployments} accepting choices</small></div><div class="admin-summary-card"><span>Player responses</span><b>${totalResponses}</b><small>Across every deployment</small></div><div class="admin-summary-card"><span>Active organisers</span><b>${activeProfiles.length}</b><small>${blockedProfiles.length} removed</small></div></div><div class="admin-command-bar"><div class="admin-view-switch" role="tablist" aria-label="Administrator views"><button class="admin-view-tab selected" type="button" role="tab" aria-selected="true" data-admin-view="deployments">Deployments <span>${missions.length}</span></button><button class="admin-view-tab" type="button" role="tab" aria-selected="false" data-admin-view="organisers">Organisers <span>${activeProfiles.length}</span></button></div><label class="admin-search"><span>Search</span><input id="adminSearchInput" type="search" placeholder="Find a deployment, organiser or link…" autocomplete="off"></label></div><section class="admin-section admin-view-panel" data-admin-panel="deployments"><div class="admin-section-head"><div><div class="eyebrow">Operations</div><h2>All deployments</h2><p class="sub">Open a deployment to manage it. Expand its live allocation only when you need the full station view.</p></div></div><div class="grid cards admin-global-deployment-list">${missions.length?missions.map(m=>adminGlobalDeploymentRow(m)).join(""):`<section class="empty-state"><h2>No deployments yet</h2><p>Create a deployment yourself or wait for an organiser to create one.</p></section>`}</div></section><section class="admin-section admin-view-panel hidden" data-admin-panel="organisers"><div class="admin-section-head"><div><div class="eyebrow">Accounts</div><h2>Organisers</h2><p class="sub">Review account ownership, deployments and access. Removed accounts stay archived until restored.</p></div></div><div class="admin-organiser-list">${activeOrganiserCards}</div>${removedOrganisers}</section><section id="adminSearchEmpty" class="empty-state compact hidden"><h2>No matches</h2><p>Try a deployment name, organiser, ship or custom link.</p></section>`;
  ...
  document.querySelectorAll("[data-copy]").forEach(b=>b.onclick=()=>copyMissionLink(b.dataset.copy,b,b.dataset.inviteSlug||""));
  ...
  const searchInput=$("#adminSearchInput"),emptyState=$("#adminSearchEmpty");
  let activeAdminView="deployments";
  function filterAdminView(){const panel=document.querySelector(`[data-admin-panel="${activeAdminView}"]`),needle=searchInput.value.trim().toLowerCase(),selector=activeAdminView==="deployments"?".admin-global-deployment-row":".admin-organiser-card";let visible=0;panel.querySelectorAll(selector).forEach(item=>{const match=!needle||(item.dataset.adminSearch||"").includes(needle);item.classList.toggle("hidden",!match);if(match)visible++;});emptyState.classList.toggle("hidden",visible>0||!needle);}
  document.querySelectorAll("[data-admin-view]").forEach(button=>button.onclick=()=>{activeAdminView=button.dataset.adminView;document.querySelectorAll("[data-admin-view]").forEach(x=>{const selected=x===button;x.classList.toggle("selected",selected);x.setAttribute("aria-selected",String(selected));});document.querySelectorAll("[data-admin-panel]").forEach(panel=>panel.classList.toggle("hidden",panel.dataset.adminPanel!==activeAdminView));searchInput.placeholder=activeAdminView==="deployments"?"Find a deployment, organiser, ship or link…":"Find an organiser by name or email…";filterAdminView();});
  searchInput.addEventListener("input",filterAdminView);
}
